...
int prestamo_buscarID(Prestamo array[], int size, int valorBuscado, int* posicion)                    //cambiar prestamo
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].id_prestamo==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int prestamo_alta(Prestamo array[], int size, int* contadorID)                          //cambiar prestamo
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].id_prestamo=*contadorID;                                                       //campo ID
...
//            utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,1,array[posicion].varLongString);                 //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n fecha: %s\n",
                   posicion, array[posicion].id_prestamo,array[posicion].fecha);
            retorno=0;
        }
    }
    return retorno;
...
int prestamo_baja(Prestamo array[], int sizeArray)                                      //cambiar prestamo
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].id_prestamo=0;                                                                   //cambiar campo id
//            array[posicion].varInt=0;                                                               //cambiar campo varInt
//            array[posicion].varFloat=0;                                                             //cambiar campo varFloat
            strcpy(array[posicion].fecha,"");                                                   //cambiar campo fecha
...
int prestamo_bajaValorRepetidoInt(Prestamo array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        {
            if(array[i].id_prestamo==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                array[i].isEmpty=1;
                array[i].id_prestamo=0;                                                                   //cambiar campo id
//                array[i].varInt=0;                                                               //cambiar campo varInt
//                array[i].varFloat=0;                                                             //cambiar campo varFloat
                strcpy(array[i].fecha,"");                                                   //cambiar campo fecha
...
//    char bufferLongString[TEXT_SIZE];                           //cambiar campo varLongString
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            ...
            bufferId=array[i].id_prestamo;                                   //cambiar campo id
...
//            strcpy(bufferLongString,array[i].varLongString);          //cambiar campo varLongString
            ...
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                ...
                array[j + 1].id_prestamo=array[j].id_prestamo;                                //cambiar campo id
...
//                strcpy(array[j + 1].varLongString,array[j].varLongString);  //cambiar campo varLongString
            ...
            array[j + 1].id_prestamo=bufferId;                                        //cambiar campo id
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n fecha: %s",
                       array[i].id_prestamo,array[i].fecha);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
