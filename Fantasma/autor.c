...
int autor_buscarID(Autor array[], int size, int valorBuscado, int* posicion)                    //cambiar autor
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].id_autor==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int autor_alta(Autor array[], int size, int* contadorID)                          //cambiar autor
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            printf("\n Posicion: %d\n ID: %d\n nombre: %s\n apellido: %s",
                   posicion, array[posicion].id_autor,array[posicion].nombre,array[posicion].apellido);
            retorno=0;
        }
    }
    return retorno;
...
int autor_baja(Autor array[], int sizeArray)                                      //cambiar autor
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].id_autor=0;                                                                   //cambiar campo id
...
int autor_bajaValorRepetidoInt(Autor array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        {
            if(array[i].id_autor==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                array[i].isEmpty=1;
                array[i].id_autor=0;                                                                   //cambiar campo id
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            {       //copiar printf de alta
                printf("\n---------------\nPosicion: %d\nID: %d\nNombre: %s\nApellido: %s\n---------------\n",
                       posicion, array[posicion].id_autor,array[posicion].nombre,array[posicion].apellido);
                utn_getChar("\nModificar: N(nombre) | A(apellido) | S(salir): ","\nError",'A','Z',1,&opcion);
                switch(opcion)
                {
                    case 'N':
                        utn_getName("\n: ","\nError",1,TEXT_SIZE,1,array[posicion].nombre);                      //mensaje + cambiar campo nombre
...
//    float bufferFloat;                                          //cambiar buffer varFloat
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            ...
            bufferId=array[i].id_autor;                                   //cambiar campo id
...
//            bufferFloat=array[i].varFloat;                            //cambiar campo varFloat
            ...
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                ...
                array[j + 1].id_autor=array[j].id_autor;                                //cambiar campo id
...
//                array[j + 1].varFloat=array[j].varFloat;                    //cambiar campo varFloat
            ...
            array[j + 1].id_autor=bufferId;                                        //cambiar campo id
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n nombre: %s %s\n",
                       array[i].id_autor,array[i].nombre,array[i].apellido);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
