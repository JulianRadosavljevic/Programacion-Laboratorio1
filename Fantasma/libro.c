...
int libro_buscarID(Libro array[], int size, int valorBuscado, int* posicion)                    //cambiar libro
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].id_libro==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int libro_alta(Libro array[], int size, int* contadorID)                          //cambiar libro
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].id_libro=*contadorID;                                                       //campo ID
            array[posicion].isEmpty=0;           //mensaje + cambiar campo varInt
//            utn_getFloat("\ngetFloat: ","\nError",1,sizeof(float),0,1,1,&array[posicion].varFloat);             //mensaje + cambiar campo varFloat
            printf("\nIngrese datos del libro\n");
            utn_getUnsignedInt("\nCodigo de autor: ","\nError",1,sizeof(int),1,1,1,&array[posicion].autor);                   //mensaje + cambiar campo titulo
...
//            utn_getTexto("getTexto\n: ","\nError",1,TEXT_SIZE,1,array[posicion].varLongString);                 //mensaje + cambiar campo varLongString
            printf("\n Posicion: %d\n ID: %d\n titulo: %s",
                   posicion, array[posicion].id_libro,array[posicion].titulo);
            retorno=0;
        }
    }
    return retorno;
...
int libro_baja(Libro array[], int sizeArray)                                      //cambiar libro
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].id_libro=0;                                                                   //cambiar campo id
...
int libro_bajaValorRepetidoInt(Libro array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        {
            if(array[i].id_libro==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                array[i].isEmpty=1;
                array[i].id_libro=0;                                                                   //cambiar campo id
...
//    char bufferLongString[TEXT_SIZE];                           //cambiar campo varLongString
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            ...
            bufferId=array[i].id_libro;                                   //cambiar campo id
...
//            strcpy(bufferLongString,array[i].varLongString);          //cambiar campo varLongString
            ...
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                ...
                array[j + 1].id_libro=array[j].id_libro;                                //cambiar campo id
...
//                strcpy(array[j + 1].varLongString,array[j].varLongString);  //cambiar campo varLongString
            ...
            array[j + 1].id_libro=bufferId;                                        //cambiar campo id
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n titulo: %s\n",
                       array[i].id_libro,array[i].titulo);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
