...
int socio_buscarID(Socio array[], int size, int valorBuscado, int* posicion)                    //cambiar socio
{
    ...
    if(array!= NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                continue;
            else if(array[i].id_socio==valorBuscado)                                                   //cambiar campo ID
            {
                retorno=0;
                *posicion=i;
                break;
...
int socio_alta(Socio array[], int size, int* contadorID)                          //cambiar socio
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        {
            (*contadorID)++;
            array[posicion].id_socio=*contadorID;                                                       //campo ID
...
//            utn_getFloat("\ngetFloat: ","\nError",1,sizeof(float),0,1,1,&array[posicion].varFloat);             //mensaje + cambiar campo varFloat
            ...
            printf("\n Posicion: %d\n ID: %d\n nombre: %s\n apellido: %s",
                   posicion, array[posicion].id_socio,array[posicion].nombre,array[posicion].apellido);
            retorno=0;
        }
    }
    return retorno;
...
int socio_baja(Socio array[], int sizeArray)                                      //cambiar socio
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        else
        {
            ...
            array[posicion].id_socio=0;                                                                   //cambiar campo id
...
int socio_bajaValorRepetidoInt(Socio array[], int sizeArray, int valorBuscado) //cuando hay que dar de baja todas las posiciones en las que se encuentra ese int
{
    ...
    if(array!=NULL && sizeArray>0)
    {
        ...
        {
            if(array[i].id_socio==valorBuscado)                                                        //cambiar si no se busca por ID
            {
                array[i].isEmpty=1;
                array[i].id_socio=0;                                                                   //cambiar campo id
...
//    float bufferFloat;                                          //cambiar buffer varFloat
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
            ...
            bufferId=array[i].id_socio;                                   //cambiar campo id
...
//            bufferFloat=array[i].varFloat;                            //cambiar campo varFloat
            ...
            {                                                                                                               //buffer < campo ascendente   buffer > campo descendente
                ...
                array[j + 1].id_socio=array[j].id_socio;                                //cambiar campo id
...
//                array[j + 1].varFloat=array[j].varFloat;                    //cambiar campo varFloat
            ...
            array[j + 1].id_socio=bufferId;                                        //cambiar campo id
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n nombre: %s\n apellido: %s",
                       array[i].id_socio,array[i].nombre,array[i].apellido);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
